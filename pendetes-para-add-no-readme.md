Achei legal a arte deste site aqui, tudo combina, nada exagerado.
https://www.hostinger.com/br/horizons

Gostei dos textos das propagandas deste aqui:
https://paxdigital.com.br/criacao-de-sites/
Essa frase é matadora: "Conquiste mais clientes com um site profissional." 
A segunda sessão também ficou muito legal "Três passos para seu site ficar pronto."
1. Conversa inicial
2. Criação do projeto
3. Aprovação e publicação
Terceira sessão também muito boa, com estes textos e uma foto de layout para cada um deles: "Sites profissionais para diferentes áreas de atuação."
Site para médicos, dentistas e clínicas, Site para psicólogos e terapeutas, Site para advogados e consultores, Site para viagens, turismo e lazer, Site para salões de beleza e barbearias, Site para empresas e prestadores de serviço.
Palavras chave que achei boas: Design responsivo e moderno, Seja encontrado no Google (site estruturado pra facilitar isso), Botão de WhatsApp integrado, Hospedagem e suporte.
Sessão de dúvidas, muito bom também.
Final matador: "Seu próximo cliente pode estar procurando por você agora." e aí btn pra entrar em contato "Fale com um especialista"

https://belasites.com.br/
Achei o design meio ruim, fotos de baixa qualidade, cores que não combinam tanto, fontes exageradas, muito caps lock e btn em todo canto.
Achei legal que oferta loja virtual, ainda pode ser um produto importante para alguns clientes.
Tráfego pago também é oferecido, é um nicho legal para oferecer também? tem muito retorno?
Carroséis bem genéricos, usaram plugin do wordpress?
Diversos tipos de design de sites para diversos tipos de clientes/negócios: https://belasites.com.br/sites-desenvolvidos-bela.html
Algumas palavras chave interessantes:
Sites Institucionais
Lojas Virtuais
Landing Pages
Blogs e Conteúdo
Sites Personalizados
Sites One Page
SEO Técnico
Manutenção e Suporte
Eles tem uma sessão de tecnologias utilizadas, citando php, wordpress, magento, python, vertex, faz sentido ainda ter isso hoje em dia?

https://x-apps.com.br/lp-desenvolvimento-de-aplicativo-v2
Achei interessante o carrossel de apps, tipo com a visualização das telas de apps para diferentes segmentos, no recorte dentro da telinha de um celular achei legal.
https://x-apps.com.br/
Nessa página o carrossel de clientes ficou ainda mais bonito, citando clientes como polishop, suzano e bmw.
Mas como eles tem clientes gigantes/bilionários, ele não precisa se vender muito, o porto dos clientes que tem fala por si só, por isso o site é tão minimalista.
Essa sessão de portifólio ficou linda pra cacete: https://x-apps.com.br/portfolio
Ter newsletter no site ainda vale a pena? sessão de blogs? trás real retorno agora que as pessoas acessam cada vez menos e perguntam tudo pra IAs?
É multi language, tem pt-br e en-us, acho que deveríamos ter pt-br, en-us e espanhol também, já que latam muito do Brasil também.
Pela página de serviços, eles estão mais pra outsourcing, não é isso que quero, eu vendo a solução, o produto, o pessoal é meu, nada de quererem controlar meus horários, bater ponto e tal: https://x-apps.com.br/servicos
Palavras chave interessantes: UX/UI e prototipação, Aplicações web e mobile, Integrações e APIs, Cloud e infraestrutura, Qualidade e segurança, Dados e analytics.

https://pt.wix.com/premium-purchase-plan/dynamo
https://webrobotapps.com/planos-aplicativos/
https://www.fabricadeaplicativos.com.br/planos
https://base44.com/pt/pricing
https://www.figma.com/pt-br/solucoes/criador-de-aplicativos-ia/
https://www.figma.com/pt-br/pricing/
https://www.figma.com/community/make
Estes aqui são importantes para explormos precificação, a deles obviamente é baixa, pois não é uma solução customizada, mas sim de larga escala, o que podemos aprender com isso, como devemos nos posicionar?
Devemos reforçar o que? falta de suporte? nem tudo sai como o planejado?
brechas de segurança?
não propriedade do código/produto que geralmetne pertence a essas plataformas?

https://octopusmediahub.com/sites-landing-pages
Já este aqui tem uma propaganda mais agressiva:
Vendas 24h por dia / 7 dias por semana
Seu site trabalha enquanto você dorme, pense nisso
Escalabilidade garantida
Crescimento previsível
Redução de custo por lead
Mais conversões com o mesmo tráfego
SEO técnico completo
Velocidade otimizada
Integrações essenciais
UX pensada para navegação intuitiva
Landing Pages para tráfego pago
Copywriting focado em venda
Design responsivo premium
Arquitetura estratégica de conversão
Apareça no Google. Ganhe credibilidade. Converta visitantes em clientes.Escolha o plano ideal e pare de perder oportunidades para quem já investiu em presença digital profissional.
Hospedagem profissional que garante performance máxima.
### Seção Planos
**TABELA COMPARATIVA DE HOSPEDAGEM**
## PLANO START
- 3GB de armazenamento
- 1 site e 1 domínio
- 1 conta de e-mail institucionais  
  ex.: contato@seudominio.com
- Certificado SSL grátis
- Suporte via Mensagens e WhatsApp
## PLANO PRO
- 7GB de armazenamento
- 1 site e até 2 domínios
- Até 1 Subdomínio
- 2 contas de e-mail institucionais  
  ex.: contato@seudominio.com
- Certificado SSL grátis
- Suporte via Mensagens e WhatsApp
## PLANO GROWTH
- 10GB de armazenamento
- Até 2 sites e 3 domínios
- Até 3 Subdomínios
- 4 contas de e-mail institucionais  
  ex.: contato@seudominio.com
- Certificado SSL grátis
- Suporte via Mensagens e WhatsApp
## PLANO ENTERPRISE
- Armazenamento personalizado
- Sites e domínios ilimitados
- Subdomínios ilimitados
- E-mails institucionais ilimitados  
  ex.: contato@seudominio.com
- Certificado SSL grátis
- Suporte via Mensagens e WhatsApp
Hospedagem Octopus: máxima performance, zero dor de cabeça
Migração gratuita:  Trazemos seu site de outro servidor sem custo
Sem taxa de setup:  Comece imediatamente, sem cobranças extras
Cancele quando quiser: Sem multa, sem burocracia 
Suporte nativo: Time brasileiro e norte-americano que entende seu negócio
Infraestrutura no Brasil e EUA:  Servidores locais = mais velocidade para seu público
Painel intuitivo:  Você controla tudo facilmente
Escalabilidade garantida: Upgrade simples conforme crescimento
Infraestrutura profissional, suporte humano, estabilidade e performance real.
Por que 80% dos sites não geram vendas?
Porque foram feitos apenas como vitrine, sem estratégia.
Na Octopus, cada site é construído com funil de conversão embutido. Não criamos páginas bonitas para sua vaidade. Criamos sistemas que transformam cliques em clientes. 
Cada elemento tem função estratégica:
Headline que prende atenção
Copy que educa e persuade
CTAs que guiam para ação
UX que elimina fricção
SEO que traz tráfego orgânico
Seus clientes estão pesquisando AGORA. Sua empresa aparece ou não?Cada dia sem site estratégico é dinheiro jogado fora. 
Clientes qualificados estão procurando soluções neste momento, e encontrando seus concorrentes. 
Garanta um site ou landing page que trabalha por você 24h por dia.
Fale com especialista
Quer virar nosso próximo case de sucesso?
Ecossistema estratégico de marketing que integra dados, criatividade e tecnologia para escalar negócios com previsibilidade.
Já a arte deles achei muito escura, muito exagerada em tamanho de fontes e tal, acho que agride os olhos.
Página interessante de serviços:
https://octopusmediahub.com/servicos/
Ao clicar nos pacotes oferencidos não tem preço, mas sim redirect para o whatsapp, isso funciona mesmo?

Achei este website interessante também:
https://www.wimagroup.com.br/services/software-sob-medida

Este também:
https://dynamicasoft.com/empresa-de-aplicativos-em-sao-paulo

Olha só a propaganda desses caras no google, isso vende?

lp.condevhouse.com
https://lp.condevhouse.com
Seu Software sob medida em 3x mais rápido. Desenvolvemos aplicativos e...
‎Software a partir de R$ 30 mil · ‎70% mais barato e mais rápido · ‎Aplicativo precificação
E no link: Desenvolvemos em 90 dias - App a partir de R$18 mil

Podemos usar o FAQ de cada um destes sites para ajudar a construir o nosso próprio.


Também gostei destes caras aqui, eles tem uma abordagem bem humana:
https://phurshell.com/

Esse daqui tem alguns textos legais:
https://www.agence.com.br/solucoes/mobile/
Por que Criar Aplicativo Móvel para sua Empresa?
Engajamento com o Cliente: Esteja sempre ao alcance dos seus clientes com experiências personalizadas e intuitivas.
Otimização Operacional: Melhore processos e reduza custos com aplicativos corporativos eficazes.
Aumento de Vendas: Potencialize suas vendas com uma presença mobile impactante.
Decisões Estratégicas: Acesse relatórios detalhados a qualquer momento, facilitando tomadas de decisão ágeis e precisas.
Por que Escolher a Agence?
26 anos de Experiência
Projetos bem-sucedidos nacionais e internacionais.
Consultoria Estratégica
Soluções que vão além do código, com foco em resultados.
Entrega Garantida
Cumprimento de prazos e qualidade assegurada.
Prototipagem
Validação de ideias com protótipos funcionais.
Hospedagem e Suporte
Servidores de alta performance e suporte 24/7.
Atualizações Contínuas
Manutenção para melhorias constantes.
Arquitetura Modular
Desenvolvimento por etapas com expansão gradual.
Foco no Usuário
Produtos intuitivos, centrados na experiência do cliente.
E eles tem uns clientes bem grandes: Latam, Hunter Douglas, Shire, Ferring Pharmaceuticals e por aí vai...

Precisamos também analisar como e quais produtos vamos oferecer:
Automações por exemplo, essa página ficou legal mas podia ser melhor:
https://www.agence.com.br/solucoes/automacao-de-processos/
Benefícios do RPA para sua Empresa:
Aumente a produtividade com automação 24/7 de tarefas repetitivas.
Otimize recursos e aumente a rentabilidade do seu negócio.
Libere sua equipe para focar em atividades estratégicas e criativas.
Elimine erros humanos e garanta consistência em operações críticas.
Elimine o retrabalho com tarefas de pouco valor.
Obtenha insights com automação de coleta e processamento de dados.
Resultados imediatos com solução de baixa complexidade técnica.
Confiabilidade garantida com operação contínua e padronizada 24/7.

Chatbot:
Pesquisa do google usada:
https://www.google.com/search?q=automa%C3%A7%C3%A3o+de+WhatsApp+para+empresas&oq=automa%C3%A7%C3%A3o+de+WhatsApp+para+empresas&gs_lcrp=EgZjaHJvbWUyBggAEEUYOdIBBzM0M2owajeoAgCwAgA&sourceid=chrome&ie=UTF-8&source=chrome.ob
https://letalk.com.br/
https://letalk.com.br/planos/
https://chatguru.com.br/
https://chatguru.com.br/planos-e-precos/
https://botconversa.com.br/en/
https://botconversa.com.br/en/#pricing
https://www.n2code.com.br/automacao-de-whatsapp-para-empresas
Gostei deste texto:
No cenário empresarial moderno, o WhatsApp transcendeu sua função de aplicativo de mensagens pessoais para se tornar um dos canais de comunicação mais diretos e eficazes entre marcas e consumidores. Com uma taxa de abertura que supera 90%, a plataforma oferece uma oportunidade inigualável de engajamento.
No entanto, gerenciar manualmente um grande volume de conversas é insustentável e sujeito a erros. É aqui que a automação de WhatsApp para empresas surge como uma solução transformadora, e a combinação com a ferramenta n8n eleva essa estratégia a um novo patamar de inteligência e eficiência.
Aqui tem muito poder: Desde a criação de um chatbot para WhatsApp que responde a perguntas frequentes até a capacidade de integrar o WhatsApp com sistemas de CRM e ERP, as possibilidades são vastas e impactantes. Este guia completo explorará como a automação de WhatsApp com n8n pode revolucionar a comunicação do seu negócio, permitindo até mesmo criar um agente de IA para WhatsApp e utilizando a API para automação de WhatsApp de forma estratégica.
O Que é a Automação de WhatsApp e Por Que Ela é Essencial?
A automação de WhatsApp consiste em utilizar softwares e plataformas para gerenciar conversas, enviar mensagens e executar tarefas dentro do aplicativo sem intervenção humana direta. Isso é feito através da API (Interface de Programação de Aplicações) do WhatsApp Business, que permite que sistemas externos se comuniquem com a plataforma de mensagens de forma programática.
Para qualquer empresa que lida com um fluxo constante de interações com clientes, a automação deixa de ser um luxo e se torna uma necessidade estratégica.
Principais benefícios da automação de WhatsApp:
Atendimento ao Cliente 24/7: Chatbots podem fornecer respostas instantâneas a dúvidas comuns a qualquer hora do dia ou da noite, garantindo que nenhum cliente fique sem resposta.
Aumento da Eficiência e Produtividade: Ao automatizar tarefas repetitivas, como qualificação de leads, agendamentos e envio de notificações, sua equipe pode se concentrar em atividades estratégicas que exigem um toque humano.
Escalabilidade Ilimitada: Uma solução automatizada pode lidar com centenas ou milhares de conversas simultaneamente, algo impossível para uma equipe humana. Isso permite que sua empresa cresça sem que o custo com atendimento ao cliente cresça na mesma proporção.
Redução de Custos Operacionais: A automação diminui a necessidade de uma grande equipe de atendimento, otimizando recursos e reduzindo custos trabalhistas.
Geração e Qualificação de Leads: Chatbots podem ser programados para fazer perguntas de qualificação, coletando informações valiosas e identificando os leads mais promissores antes de encaminhá-los para a equipe de vendas.
Personalização em Escala: Ao integrar o WhatsApp com seu CRM, é possível enviar mensagens personalizadas, como lembretes de aniversário, status de pedidos e ofertas exclusivas, fortalecendo o relacionamento com o cliente.
Achei legal isso aqui mas não gostaria de amarrar a uma ferramenta específica como n8n, assim posso usar qualquer outra similar ou melhor como crewai ou qualquer outro orquestrador ou ferrametna, mas o texto ficou legal:
N8N: A Ferramenta Revolucionária para Automação de WhatsApp
Enquanto existem diversas ferramentas de automação no mercado, o n8n se destaca por sua flexibilidade, poder e modelo de código aberto (fair-code). O n8n é uma plataforma de automação de fluxos de trabalho que permite conectar diferentes aplicações e serviços de forma visual, através de um sistema de "nós" (nodes). Isso significa que é possível criar automações complexas sem a necessidade de conhecimento avançado em programação.
Por que escolher o n8n para a automação de WhatsApp?
Flexibilidade Inigualável: Diferente de plataformas fechadas, o n8n permite a criação de fluxos de trabalho totalmente personalizados para atender às necessidades específicas do seu negócio.Você não fica preso a modelos pré-definidos e pode construir a lógica exata que sua operação exige.
‍Integrações Poderosas: A grande força do n8n é sua capacidade de integrar o WhatsApp com sistemas diversos.Com mais de 1000 integrações disponíveis, você pode conectar seu WhatsApp a CRMs (Salesforce, HubSpot), planilhas (Google Sheets, Airtable), bancos de dados, ferramentas de e-mail marketing e praticamente qualquer outra aplicação que possua uma API.
‍Custo-Benefício Superior: Por ser uma ferramenta de código aberto, o n8n pode ser auto-hospedado em seu próprio servidor, eliminando custos recorrentes de assinatura que são comuns em outras plataformas. Isso o torna uma solução extremamente econômica, especialmente para operações em escala.
‍Controle Total sobre seus Dados: Ao hospedar sua própria instância do n8n, você mantém controle total sobre seus dados e a lógica de suas automações, um fator crucial para empresas que lidam com informações sensíveis e se preocupam com a segurança e a privacidade.
https://www.n2code.com.br/automacao-de-whatsapp-para-empresas
https://www.n2code.com.br/case-studies
https://www.chatpro.com.br/precos

Precisamos adicionar, de alguma forma, também uma coisa de valor que eu sei fazer: adicionar observabilidade nos aplicativos. Não é simplesmente fazer aplicativos, tá? Como você vai adicionar observabilidade? Vai ter um centro nele? Não vai! Vai ter Firebase, vai ter Crashlytics. Então tudo isso é um conhecimento bem avançado que só realmente um desenvolvedor como eu sabe fazer. Eu preciso vender certo? Então precisa adicionar isso de alguma forma. Me ajuda a pensar em maneiras de fazer isso? 

Esse guia aqui é fodástico:
https://www.kommo.com/br/blog/chatbot-whatsapp/

Este tem muitos clientes grandes tais como rock in rio, unimed, adapta, etc:
https://botconversa.com.br/en/

Gostei da home, da frase do topo:
https://a.umbler.com/br/chatbot-talk/
O chatbot que dá super poderes para o seu WhatsApp
Qualifique seus leads, tenha um funil que vende 24h por dia e transforme o seu negócio para sempre com o atendente mais produtivo do mundo.
Será que o chatbot funciona para o meu negócio?
Todo negócio tem suas necessidades específicas, por isso o Umbler Talk é a ferramenta com mais opções e configurações disponível no mercado.
Saúde (Clínicas, Hospitais e Farmácias) — automatizando atendimentos, agendamentos e lembretes de consultas.
Educação (Cursos online, Escolas e Universidades) — escala atendimentos com perguntas corriqueiras de cadastro.
Escritórios (Contabilidade e Advocacia) — evita solicitações não tratadas, impostos atrasados e prazos perdidos.
Provedores — operação de vendas para atender com eficiência em datas de grande demanda (Black Friday, Dia das Mães etc.).
Outras empresas de serviços — fluxo de trabalho sem deixar cliente sem orçamento ou estourar prazo.
Indústrias — representantes, distribuidores, clientes e fornecedores acessam vários departamentos por um único número, com histórico de conversas.

Outros grandes players do mercado:
https://www.rdstation.com/produtos/conversas/chatbot-para-whatsapp/
https://www.zendesk.com.br/pricing/

Opções mais baratas de chatbot:
https://www.tecmundo.com.br/software/266837-melhor-chatbot-whatsapp-veja-7-opcoes.htm
https://sendpulse.com/br/pricing/messengers

Whatsapp business oferece omnichannel com integrações com insta e fb já que pertencem todos à meta?
https://whatsappbusiness.com/products/business-platform/

Interessantes as features do CRM deles:
https://www.bitrix24.com.br/tools/crm/
Mas achei caro pra assinar, as ferramentas que realmente valem a pena estão no plano de 1k reais, isso é absurdo, mas é interessante pra eu saber como precificar, mas claro, se eu precisar, eu mesmo codifico isso.
